...
//word is guessed
const gameOver = () => {
    document.getElementById('feedback-message').textContent = "Congratulations you guessed the word";
    document.getElementById('feedback-message').style.color = "blueviolet";
    inputWordElement.disabled = true;
    submitGuessButton.disabled = true;
}

//function to select the guessed random word
const guessedWord = words[Math.floor(Math.random() * words.length)].toLowerCase();
guessedWordArray = [...guessedWord];
console.log(guessedWord);

//Regex for only upper/lowercase alphabets only 
const onlyLetters = /^[A-Za-z]{5}$/;

//counter that logs the word to the game
let guessCount = 9;
let startIndex = 0;
...
submitGuessButton.onclick = () => {
    ...
    const inputWord = inputWordElement.value.trim().toLowerCase();
    const inputWordArray = [...inputWord];
    //check if the word is invalid
    if (!onlyLetters.test(inputWord)){
        document.getElementById('feedback-message').textContent = "Please enter a 5 letter word only";
        ...
        // valid 5 letter word
        for(let i=startIndex, j=0 ; i<(startIndex+5), j<5 ;i++, j++) {

            const gridItemElement = document.getElementById(`grid-item-${i+1}`);
            gridItemElement.textContent = inputWordArray[j];

            if(guessedWordArray.includes(inputWordArray[j])){

                if(guessedWordArray[j] == inputWordArray[j]){
                    gridItemElement.style.backgroundColor = "green";
                    gridItemElement.style.color = "white";
                }
                else{
                    gridItemElement.style.backgroundColor = "yellow";
                }

            }
            else{
                gridItemElement.style.backgroundColor = "gray";
            }
        ...
        inputWordElement.value = '';
        guessCount --;
        if(guessedWord == inputWord){
                gameOver();
        }
        if(guessCount == 0){
            document.getElementById('feedback-message').textContent = `Game over, the word was ${guessedWord}`;
            submitGuessButton.onclick = () => null;
        }
